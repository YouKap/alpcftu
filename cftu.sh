...
install_cf() {
    clear
    echo -e "${BLUE}正在安裝 Cloudflared 環境 (Alpine)...${PLAIN}"
    mkdir -p /etc/cloudflared
    
    echo -e "${YELLOW}更新軟體源並安裝必要套件...${PLAIN}"
    apk update && apk add --no-cache bash curl wget sudo gawk nano openrc coreutils libc6-compat
    
    # 根據架構判斷下載適配的 Cloudflared 二進位檔案
    ARCH=$(uname -m)
    if [ "$ARCH" = "x86_64" ]; then
        ...
        echo -e "${RED}錯誤: 不支援的系統架構 ($ARCH)${PLAIN}"
        sleep 2
        return
    fi

    echo -e "${YELLOW}正在下載 Cloudflared 二進位檔案...${PLAIN}"
    curl -L "$CF_URL" -o /usr/bin/cloudflared
    chmod +x /usr/bin/cloudflared
    
    echo -e "${GREEN}Cloudflared 安裝完成！${PLAIN}"
...
EOF
    ...
    # 寫入 OpenRC 服務腳本
    cat > /etc/init.d/${TUN_NAME} << EOF
#!/sbin/openrc-run

name="Cloudflared Tunnel \${RC_SVCNAME}"
description="Cloudflared Tunnel Service"
command="/usr/bin/cloudflared"
command_args="tunnel --config ${CF_DIR}/${TUN_NAME}.yml run"
command_background="yes"
pidfile="/run/\${RC_SVCNAME}.pid"
output_log="/var/log/\${RC_SVCNAME}.log"
error_log="/var/log/\${RC_SVCNAME}_error.log"
...
EOF
    ...
    chmod +x /etc/init.d/${TUN_NAME}
    rc-update add ${TUN_NAME} default >/dev/null 2>&1
    rc-service ${TUN_NAME} start
...
manage_tunnels() {
    while true; do
        ...
        for i in "${!TUNNELS[@]}"; do
            if rc-service "${TUNNELS[$i]}" status 2>/dev/null | grep -q "started"; then
                S_TEXT="${GREEN}● 運行中${PLAIN}"
            else
                S_TEXT="${RED}○ 已停止${PLAIN}"
            fi
        ...
        case $ACT in
            1) tail -n 50 /var/log/${TARGET}.log /var/log/${TARGET}_error.log 2>/dev/null; read -rp "回車返回..." d < /dev/tty ;;
            2) rc-service ${TARGET} restart && echo -e "${GREEN}重啟成功${PLAIN}"; sleep 1 ;;
            3) nano ${CF_DIR}/${TARGET}.yml < /dev/tty && rc-service ${TARGET} restart; echo -e "${GREEN}已重啟${PLAIN}"; sleep 1 ;;
            4) 
               ...
               if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then
                   ...
                   rc-service ${TARGET} stop 2>/dev/null
                   rc-update del ${TARGET} default 2>/dev/null
                   ...
                   rm -f /etc/init.d/${TARGET} ${CF_DIR}/${TARGET}.yml ${CREDS_DIR}/${UUID_DEL}.json
                   rm -f /var/log/${TARGET}.log /var/log/${TARGET}_error.log
...
show_status() {
    ...
    if [[ ${#TUNNELS[@]} -eq 0 ]]; then
        ...
        for t in "${TUNNELS[@]}"; do
            if rc-service "${t}" status 2>/dev/null | grep -q "started"; then
                echo -e "通道 [${GREEN}${t}${PLAIN}] : ${GREEN}正常運行中${PLAIN}"
            else
                echo -e "通道 [${RED}${t}${PLAIN}] : ${RED}未運行${PLAIN}"
            fi
        done
    fi
    echo -e "-------------------------------------------------"
...
clean_and_restart() {
    ...
    if [ ${#TUNNELS[@]} -eq 0 ]; then
         ...
         for t in "${TUNNELS[@]}"; do
             # OpenRC 環境下清理自定義輸出的日誌
             > /var/log/${t}.log
             > /var/log/${t}_error.log
             echo -ne "正在重啟通道 ${CYAN}${t}${PLAIN} ... "
             rc-service ${t} restart >/dev/null 2>&1 && echo -e "${GREEN}[成功]${PLAIN}" || echo -e "${RED}[失敗]${PLAIN}"
         done
    fi
    ...
    if [[ "$confirm" == "YES" ]]; then
        ...
        if [ ${#TUNNELS[@]} -eq 0 ]; then
            ...
            for t in "${TUNNELS[@]}"; do
                ...
                rc-service ${t} stop 2>/dev/null
                rc-update del ${t} default 2>/dev/null
                ...
                rm -f /var/log/${t}.log /var/log/${t}_error.log
            done
...
while true; do
    ...
    echo -e "${BLUE}=================================================${PLAIN}"
    echo -e "  🚀 ${GREEN}Cloudflare Tunnel (cftu) 管理面板 (Alpine)${PLAIN}"
